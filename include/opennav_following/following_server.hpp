...
// nav2_ros_common) and to use a self-contained FollowObject action instead
// of nav2_msgs. See README.md for details of what changed and why.
//
...
#include "opennav_following/action/follow_object.hpp"
...
{
public:
  using FollowObject = opennav_following::action::FollowObject;
...
protected:
  ...
  double filter_coef_;
  // Max angular velocity/acceleration used for the in-place search rotation.
  // opennav_docking::Controller in this ROS distribution has no rotate-in-place
  // helper, so this is a small self-contained accel-limited P controller.
  double rotate_angular_velocity_;
  double rotate_angular_acceleration_;
